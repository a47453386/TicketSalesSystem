    ...
    public class Background : BackgroundService
    {
        ...
        private async Task UpdateProgrammeStatusAsync(TicketsContext context)
        {
            ...
            var toOnSale = await context.Programme
                .Where(s => s.ProgrammeStatusID == "B" && s.Session.Any(s => s.SaleStartTime <= now && s.SaleEndTime > now))
                .ToListAsync();
            foreach (var s in toOnSale) s.ProgrammeStatusID = "O";
            ...
            foreach (var s in toEnded) s.ProgrammeStatusID = "E";
            if (toOnSale.Any() || toEnded.Any())
            {
                await context.SaveChangesAsync();
